...
- 字体系统完善：运行数据目录自动提供 `fonts/`，用户可放入 `.ttf/.otf/.ttc/.woff/.woff2`，Page 按文件名或相对路径选择，实际浏览器渲染 HTML 会内嵌完整用户字体表并读取同一字体。
...
Typst mode is browser-free: Page saves a fine-grained `render_snapshot` containing a full-card preview raster, preview geometry, capture scale, text metrics, CSS `text-transform`, transparent text raster fallback layers, measured text line boxes, measured grapheme boxes, padding, letter spacing, colors, boxes, images and font stacks, and Typst compiles that snapshot directly to PNG. The full-card raster is used first for maximum parity with Page; structured boxes/text remain as fallback. Transparent preview-only layers are skipped instead of being converted to black fills, while real RGBA card colors keep their alpha. Text fallback is restored from the saved transparent raster layer first, then grapheme boxes, then line boxes, to avoid unexpected re-wrapping, font fallback drift, or uppercase/lowercase drift. It also passes the plugin `fonts/` directory to Typst so selected custom fonts can be resolved by the Typst engine; Page waits for loaded fonts and embeds the same user `@font-face` CSS into the saved preview raster so the saved image does not fall back to a different font. Old menus without `render_snapshot` still use fallback Typst layout; reopen and save them in Page to generate the pixel-oriented snapshot. `render_scale` still controls the final image resolution; in `typst` mode, a saved full-card raster can fill the render cache immediately after save, and unchanged menus keep using the existing cached PNG path instead of recompiling.
