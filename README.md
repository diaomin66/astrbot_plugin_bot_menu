...
- 多菜单方案：支持新建、复制、软删除、回收站恢复、永久删除、导入、导出（含内联资产包）
- 资产中心：背景图保存到本地资产库并按 hash 去重，支持最近资产查看、未使用资产删除、引用菜单展示，旧菜单内联背景继续兼容
- 可视化编辑：标题、副标题、分组、菜单项、图标、指令、描述、启用状态，支持分组和菜单项复制、上移、下移、折叠、桌面拖拽排序
- 编辑安心体验：未保存状态提示、离开/切换菜单确认、本地草稿自动保存与恢复/丢弃提示、保存前自动快照、历史版本一键恢复
- 大菜单运营：前端即时校验、保存前滚动到首个错误、分组/菜单项折叠状态记忆、菜单项搜索过滤、批量选择/启用/禁用/复制/删除/移动
- 效率工具：撤销/重做、Ctrl+S 保存、Ctrl+Z/Y 撤销重做、`/` 聚焦搜索、命令面板、紧凑/标准/宽松 UI 密度
- 样式配置：主题预设、主色、背景色、自定义背景图（不限上传尺寸）、前景菜单透明度（0%-100%）、卡片色、文字色、辅助文字色、字体、阴影、边框、水印、智能/手动宽度、每行卡片数、智能/自定义分组间距（0-200，0 为完全贴合）、圆角、更新时间
- 样式管理：主题预设卡片预览、复制当前样式到其他菜单、一键重置样式且不删除菜单内容、对比度检查与一键修复颜色、多设备预览
...
`menus.json`、`assets.json`、`routing.json` 使用原子写入与滚动备份；删除菜单默认进入回收站。
