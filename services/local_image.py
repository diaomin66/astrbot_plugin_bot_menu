...
def render_menu_image(menu: dict[str, Any], output_dir: str | Path, *, default_width: int = 900) -> str:
    """Render a menu to a local PNG file without relying on AstrBot's remote T2I service."""
    ...
    font_regular = _font(20)
    font_small = _font(17)
    font_mono = _font(17)
    font_title = _font(46)
    font_section = _font(25)
    font_label = _font(22)
    font_badge = _font(18)
    font_icon = _font(28, emoji=True)
    ...
    y = margin + shell_pad + 12
    y += 38  # badge
    y += 16 + _text_height(font_title, menu.get("title") or "Bot 功能菜单", content_width)
    subtitle = str(menu.get("subtitle") or "")
    if subtitle:
        y += 8 + _wrapped_height(subtitle, font_regular, content_width, 27)
    y += 24
    ...
    for section in menu.get("sections", []):
        ...
        row_heights = []
        for row in rows:
            row_heights.append(max(_item_height(item, item_width, font_label, font_mono, font_small) for item in row))
        section_height = 22 + 30 + 16 + sum(row_heights) + item_gap * max(0, len(row_heights) - 1) + 22
        section_layouts.append({"section": section, "rows": rows, "row_heights": row_heights, "height": section_height})
        y += section_height + 18

    footer_text = str(menu.get("footer") or "")
    y += 26 if footer_text or style.get("show_updated_at", True) else 0
    height = int(y + margin + shell_pad)

    image = Image.new("RGB", (width, height), bg)
    draw = ImageDraw.Draw(image)
    _draw_soft_background(draw, width, height, primary, bg)
    shell = (margin, margin, width - margin, height - margin)
    draw.rounded_rectangle(shell, radius=radius, fill=(255, 255, 255), outline=_mix(muted, (255, 255, 255), 0.72), width=1)
    draw.rounded_rectangle((margin, margin, width - margin, margin + 8 + radius), radius=radius, fill=primary)
    draw.rectangle((margin, margin + 8, width - margin, margin + 8 + radius), fill=(255, 255, 255))

    x = margin + shell_pad
    y = margin + shell_pad + 12
    badge_text = f"📋 {menu.get('name') or menu.get('id') or '菜单'}"
    badge_w = min(content_width, _text_width(font_badge, badge_text) + 26)
    draw.rounded_rectangle((x, y, x + badge_w, y + 38), radius=19, fill=_mix(primary, (255, 255, 255), 0.88))
    draw.text((x + 13, y + 8), badge_text, font=font_badge, fill=primary)
    y += 54

    y = _draw_wrapped(draw, menu.get("title") or "Bot 功能菜单", (x, y), font_title, text, content_width, 52)
    if subtitle:
        y += 8
        y = _draw_wrapped(draw, subtitle, (x, y), font_regular, muted, content_width, 29)
    y += 24
    ...
    for layout in section_layouts:
        ...
        draw.rounded_rectangle((x + 22, y + 22, x + 32, y + 50), radius=5, fill=primary)
        draw.text((x + 44, y + 20), str(section.get("title") or "分组"), font=font_section, fill=text)
        y += 68
        for row, row_height in zip(layout["rows"], layout["row_heights"], strict=False):
            item_x = x + 22
            for item in row:
                _draw_item(draw, item, item_x, y, item_width, row_height, radius, primary, text, muted, font_icon, font_label, font_mono, font_small)
                item_x += item_width + item_gap
            y += row_height + item_gap
        y = bottom + 18

    if footer_text or style.get("show_updated_at", True):
        footer_y = height - margin - shell_pad - 6
...
def _draw_item(draw, item, x, y, width, height, radius, primary, text, muted, font_icon, font_label, font_mono, font_small) -> None:
    ...
    draw.rounded_rectangle((x, y, x + width, y + height), radius=max(14, radius - 8), fill=fill, outline=_mix(muted, (255, 255, 255), 0.82))
    draw.rounded_rectangle((x + 14, y + 14, x + 58, y + 58), radius=14, fill=_mix(primary, (255, 255, 255), 0.88))
    draw.text((x + 22, y + 21), str(item.get("icon") or "•")[:2], font=font_icon, fill=primary)
    text_x = x + 70
    line_y = y + 14
    ...
    if command:
        command_y = line_y + 28
        command_lines = _wrap(command, font_mono, width - 84, max_lines=2)
    ...
    for line in _wrap(desc, font_small, width - 84, max_lines=3):
        draw.text((text_x, line_y), line, font=font_small, fill=sub)
        line_y += 22


def _item_height(item, width, font_label, font_mono, font_small) -> int:
    desc_lines = len(_wrap(str(item.get("description") or ""), font_small, width - 84, max_lines=3))
    command_lines = len(_wrap(str(item.get("command") or ""), font_mono, width - 84, max_lines=2))
    return max(88, 28 + 28 + command_lines * 22 + desc_lines * 22)


def _draw_soft_background(draw, width, height, primary, bg) -> None:
    for i in range(0, 260, 8):
        ratio = i / 260
        color = _mix(primary, bg, 0.65 + ratio * 0.35)
        draw.ellipse((-120 - i // 3, -140 - i // 3, 280 + i, 260 + i), outline=color, width=8)
...
def _text_height(font, text: str, width: int) -> int:
    return len(_wrap(text, font, width)) * 52
...
def _font(size: int, *, emoji: bool = False):
